...
export default function App() {
  ...
  const handleSaveCollar = async (collarData: Partial<GPSCollar>): Promise<boolean> => {
    try {
      const url = editingCollar ? `/api/collars/${editingCollar.id}` : '/api/collars';
      ...
      if (!res.ok) {
        const message = payload?.details
          ? `${payload?.error || 'Erreur lors de l’enregistrement.'} (${payload.details})`
          : (payload?.error || `Erreur HTTP ${res.status}`);
        console.error('Erreur enregistrement collier:', payload);
      ...
      showNotification('Impossible de joindre le serveur. Vérifie Vercel et les variables Supabase.');
      return false;
    }
  };
