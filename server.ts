...
// 13bis. Manual position entry (dev-local mirror of POST /api/collars/:id/position
// in api/[...path].ts) — keeps the local Express server in parity with the real
// Vercel/Supabase pipeline for testing zone-exit + low-battery detection.
app.post('/api/collars/:id/position', (req, res) => {
  const { id } = req.params;
  const collar = collars.find(c => c.id === id);
  if (!collar) return res.status(404).json({ error: 'Collier introuvable' });
  ...
  const batteryPercent = req.body.batteryPercent !== undefined && req.body.batteryPercent !== null && req.body.batteryPercent !== ''
    ? Number(req.body.batteryPercent)
    : null;

  collar.currentLat = lat;
  collar.currentLng = lng;
  collar.lastUpdate = new Date().toISOString();
  if (batteryPercent != null && Number.isFinite(batteryPercent)) collar.batteryLevel = batteryPercent;

  const assignedZone = zones.find(z => z.id === collar.activeZoneId)
    || zones.find(z => z.assignedCollarIds.includes('all') || z.assignedCollarIds.includes(collar.id));

  const zoneCheck: { checked: boolean; inside: boolean; alerted: boolean } = { checked: false, inside: true, alerted: false };
  if (assignedZone && assignedZone.active) {
    const inside = isPointInZone(lat, lng, assignedZone);
    zoneCheck.checked = true;
    zoneCheck.inside = inside;
    if (!inside && collar.status === 'inside_zone') {
      collar.status = 'out_of_zone';
      const dist = Math.round(getDistanceMeters(lat, lng, assignedZone.centerLat, assignedZone.centerLng));
      ...
      zoneCheck.alerted = true;
    } else if (inside && collar.status === 'out_of_zone') {
      collar.status = 'inside_zone';
    }
  }

  // Une seule alerte batterie tant qu'une alerte ACTIVE (non acquittée) existe déjà.
  const batteryCheck: { checked: boolean; low: boolean; alerted: boolean } = { checked: false, low: false, alerted: false };
  if (batteryPercent != null && Number.isFinite(batteryPercent)) {
    batteryCheck.checked = true;
    if (batteryPercent < 15) {
      batteryCheck.low = true;
      const alreadyAlerted = alertsHistory.some(a => a.collarId === collar.id && a.type === 'LOW_BATTERY' && a.status === 'ACTIVE');
      if (!alreadyAlerted) {
        alertsHistory.unshift({
          ...
          type: 'LOW_BATTERY',
          status: 'ACTIVE',
          message: `ALERTE BATTERIE FAIBLE : ${collar.sheepName} est à ${batteryPercent}% de batterie.`
        });
        batteryCheck.alerted = true;
      }
    }
  }
  ...
  res.status(201).json({ ok: true, collarId: collar.id, zoneCheck, batteryCheck });
});
